...
export const SuggestionsPanel: React.FC<Props> = ({ suggestions, students, onApplySuggestion }) => {
  ...
  if (suggestions.length === 0) {
    return (
      <div className="bg-gradient-to-b from-white to-gray-50 border-l border-gray-200 w-80 shrink-0 flex flex-col shadow-lg">
  ...
  return (
    <div className="bg-gradient-to-b from-white to-gray-50 border-l border-gray-200 w-80 shrink-0 flex flex-col shadow-lg h-full overflow-hidden">
      <div className="p-6 border-b border-gray-200 bg-white shadow-sm z-10">
        ...
        <div className="flex gap-2 mt-2">
           {errors.length > 0 && <span className="text-xs px-2 py-0.5 bg-red-100 text-red-700 rounded-full font-medium">{errors.length} 错误</span>}
           {warnings.length > 0 && <span className="text-xs px-2 py-0.5 bg-amber-100 text-amber-700 rounded-full font-medium">{warnings.length} 警告</span>}
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin scrollbar-thumb-gray-200">
        ...
        {errors.length > 0 && (
            <div className="space-y-3">
                ...
                {errors.map((s, idx) => (
                    <div key={`err-${idx}`} className="bg-white border-l-4 border-red-500 rounded-r-lg shadow-sm p-3 hover:shadow-md transition-shadow group">
                        ...
                        <div className="text-sm text-gray-600 mb-2 pl-4">
                            <span className="font-medium text-gray-900">{getStudentName(s.conflict.studentId)}</span>
        ...
        {warnings.length > 0 && (
            <div className="space-y-3 pt-2">
                ...
                {warnings.map((s, idx) => (
                    <div key={`warn-${idx}`} className="bg-white border-l-4 border-amber-400 rounded-r-lg shadow-sm p-3 hover:shadow-md transition-shadow">
                        <div className="flex justify-between items-start mb-1">
                            <div className="font-semibold text-gray-800 text-sm flex items-center gap-1">
